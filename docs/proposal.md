...
A core design principle is **low-barrier adoption**: any website can participate in ACT without bilateral registration, API key exchange, or shared secrets with Global Agents. The protocol relies on public-key infrastructure and short-lived, unguessable session tokens rather than bilateral agreements. Local Agents are expected to maintain a unilateral allowlist of trusted Global Agent origins (see §4.1) — a one-time configuration step analogous to configuring trusted CAs or SSO identity providers, requiring no coordination with the Global Agent. This simplicity comes with deliberate trade-offs in caller authentication (see §4.1), which are documented throughout the spec.
